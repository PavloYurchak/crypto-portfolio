...
namespace CryptoPorfolio.API.Controller
{
    public class UserAssetsController : AbstractController
    {
        ...
        [HttpDelete("user-asset/{assetId}")]
        [ProducesResponseType(typeof(bool), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [Authorize]
        public async Task<IActionResult> DeleteUserAsset([FromRoute] int assetId, CancellationToken cancellationToken)
        {
            if (!TryGetUserId(out var userId))
            {
                return Unauthorized();
            ...
            return await HandleRequest(new DeleteUserAsset(userId, assetId), cancellationToken);
        }
